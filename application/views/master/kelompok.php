	...
	<div class="wrapper">
  	...
  	<div class="content-wrapper">
	    <section class="content-header">
      	<h1>List Master Data Kelompok Tindakan</h1>
      	<ol class="breadcrumb">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Form Master Data Kelompok Tindakan</h4>
          </div>
          <div class="modal-body">
            ...
            <form class="form-horizontal">
              <div class="form-group">
                <label for="unit" class="col-sm-4 control-label">Nama Kelompok Tindakan</label>
                <div class="col-sm-8">
                  <input type="hidden" id="id" value="0">
                  <input type="text" class="form-control" id="nama" placeholder="Nama Kelompok Tindakan">
                </div>
              </div>
            </form>
          </div>
...
<script>
  $(document).ready(function () {
    ...
    $('#save').click(function(){
      ...
      if(nama === ''){
        ...
        $('#alert span').html('Nama Kelompok Tindakan masih kosong!');
        $('#alert').fadeIn(1500);
        $('#alert').fadeOut(5000);
        $('#nama').focus();
        return false;
      ...
      var id = $('#id').val();
      if(id == 0 && act === 'simpan'){
        var url = '<?php echo base_url();?>master/act/kelompok/save';
        var data = {nama : nama};
      }
      else if(id != 0 && act === 'update'){
        var url = '<?php echo base_url();?>master/act/kelompok/upd';
  ...
  function edit(id){
    $.get('<?php echo base_url();?>master/act/kelompok/get/'+id,
      function(data) {
        if(data.result == 1){
          $('#id').val(id);
          $('#nama').val(data.nama);
  ...
  function hapus(id){
    if(confirm('Apakah Anda Yakin Ingin Menghapus Data Ini?')){
      $.get('<?php echo base_url();?>master/act/kelompok/del/'+id,
        function(data) {
          if(data.result == 1){
            $('#modal-alert').removeAttr('class');
