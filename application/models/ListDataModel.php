...
class ListDataModel extends CI_Model {
	...
	public function view($filter = 0){
		...
		$query = $this->db->query("select mp.id pelId,mp.nama pelNama,mp.alamat pelAlamat,mp.hp pelHp,mp.sex pelSex,mp.tempat pelTempat,
									...
									if(adm.pendamping='Y','ADA','TDK') pendamping,if(adm.ktp='Y','ADA','TDK') ktp,
									if(adm.shift='Y','ADA','TDK') shift,adm.no_str noStr,adm.hasil admHasil,skor.id skorId,
	...
	public function del($id){
		$v = 0;
		$this->db->query("delete from m_pelamar where id = $id");
		$res = $this->db->affected_rows();
		if($res){
			$this->db->query("delete from t_syarat_adm where pelamar_id = $id");
			$res1 = $this->db->affected_rows();
			if($res1){
				$this->db->query("delete from t_skorsing where pelamar_id = $id");
